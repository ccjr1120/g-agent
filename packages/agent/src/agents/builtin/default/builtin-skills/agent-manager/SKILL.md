...
| **skills** | 始终建议包含 `memory`（记住用户偏好等基础能力）。除非用途明确不需要（如纯一次性翻译 agent），否则 memory 是合理默认。每增加一个 builtin-skill，须同步修订 system.md |
...
**增删技能**：
- 添加技能：用户描述技能用途 → 生成 SKILL.md → **同步检查并更新 system.md**（补充 Skills first、能力边界或与新 skill 相关的说明；builtin-skills 正文虽会在运行时自动嵌入提示词，原则性描述仍须在 system.md 中维护）→ 预览 → 确认 → `write` 到对应目录
...
- **增删 builtin-skills 时必须同步更新 system.md**（原则性说明与 skill 清单保持一致；skill 正文由运行时自动嵌入，system.md 仍须人工维护）
